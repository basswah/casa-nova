...
export const PurchaseInvoiceDetail = ({ orderId, onClose }: PurchaseInvoiceDetailProps) => {
  ...
  if (orderLoading) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--clr-overlay)] backdrop-blur-sm p-4">
        <div className="bg-brand-dark/95 backdrop-blur-xl rounded-3xl p-6 md:p-8 w-full max-w-2xl mx-auto border border-white/[0.05] shadow-[inset_0_1px_0_rgba(255,255,255,0.05),inset_0_-1px_0_rgba(0,0,0,0.1),0_8px_32px_rgba(0,0,0,0.3)] animate-pulse">
  ...
  if (!order) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--clr-overlay)] backdrop-blur-sm p-4">
        <div className="bg-brand-dark/95 backdrop-blur-xl rounded-3xl p-6 md:p-8 w-full max-w-md mx-auto border border-white/[0.05] shadow-[inset_0_1px_0_rgba(255,255,255,0.05),inset_0_-1px_0_rgba(0,0,0,0.1),0_8px_32px_rgba(0,0,0,0.3)] text-center">
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-brand-dark/95 backdrop-blur-xl rounded-3xl p-6 md:p-8 w-full max-w-3xl mx-auto border border-white/[0.05] shadow-[inset_0_1px_0_rgba(255,255,255,0.05),inset_0_-1px_0_rgba(0,0,0,0.1),0_8px_32px_rgba(0,0,0,0.3)] max-h-[90dvh] md:max-h-[85dvh] overflow-y-auto relative"
      ...
      >
        ...
        {error && (
          <div className="mb-5 p-4 bg-red-900/20 border border-red-800/30 rounded-xl text-red-400 text-sm flex items-start gap-2.5">
        ...
        {order.status === 'received' && (
          <div className="mt-6 pt-6 border-t border-brand-border/50">
            <button
              ...
              disabled={deleting}
              className="w-full py-3 bg-red-600 text-white rounded-xl text-sm font-semibold hover:bg-red-500 transition-all duration-300 ease-out-expo disabled:opacity-40 disabled:cursor-not-allowed active:scale-[0.97] flex items-center justify-center gap-2"
            >
              <XCircle size={18} weight="bold" />
              {deleting ? t('common.cancelling') : t('purchases.cancelInvoice')}
            </button>
          </div>
        )}
      </motion.div>
